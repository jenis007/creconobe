...
class MentalHealth extends StatefulWidget {
  ...
  @override
  State<MentalHealth> createState() => _MentalHealth();
}

class _MentalHealth extends State<MentalHealth> {
  final logger = Logger();
          ...
          children: [
            Container(
              ...
              child: Text(
                'Explore Meditation',
                style: TextStyle(
                    color: const Color(0xFF4A4A4A), fontSize: 16, fontFamily: GoogleFonts.manrope().fontFamily),
              ),
            ),
            InkWell(
                onTap: () {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          SearchUi(audioList.data, 'MentalHealth'), //const PlayList( tag: 'Playlists',title:'Podcast'),
                    ),
                  );
                },
                child: Container(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return const SizedBox(
                        height: 100, child: Center(child: Text("Something went wrong. Please try again !")));
                  }
                }),
            Container(
                              ...
                              onTap: () {
                                if (audioList.data[index].playable) {
                                  if (_ctr.modelMusic.value.id.isEmpty ||
                                      ...
                                      _ctr.playListTitle != 'MentalHealth') {
                                    _ctr.playMusic('MentalHealth', audioList.data, index);
                                  }
                                  Get.to(() => MusicPlayer());
                                } else {
                      ...
                      return   SizedBox(
                        ...
                        child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int index){
                              ...
                              return   ListTile(
                                onTap: (){
                                  if(_ctr.modelMusic.value.id.isEmpty || audioList.data[index].id.toString() != _ctr.modelMusic.value.id  || (audioList.data[index].id.toString() == _ctr.modelMusic.value.id) && (!_ctr.modelMusic.value.isPlay) || _ctr.playListTitle != 'MentalHealth'){
                                    _ctr.playMusic('MentalHealth',audioList.data,index);
                                  }
                                  Get.to(() => MusicPlayer());
  ...
  Future<AudioPlayListModel> getAudioFiles() async {
    ...
    return apiInterface.getAllPlaylist(token!, "Meditation", "");
  }
}
