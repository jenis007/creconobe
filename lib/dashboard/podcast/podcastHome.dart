...
class PodcastHome extends StatefulWidget {
  ...
  @override
  State<PodcastHome> createState() => _PodcastHome();
}

class _PodcastHome extends State<PodcastHome> {
  final logger = Logger();
          ...
          InkWell(
              onTap: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        SearchUi(audioList.data, "PodcastHome"), //const PlayList( tag: 'Playlists',title:'Podcast'),
                  ),
                );
              },
              child: Container(
          ...
          FutureBuilder<AudioPlayListModel>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  audioList = snapshot.data!;
                  logger.d("podcast list ==== ${audioList.data.length}");
                            ...
                            onTap: () {
                              if (audioList.data[index].playable) {
                                if (_ctr.modelMusic.value.id.isEmpty ||
                                    ...
                                    _ctr.playListTitle != 'PodcastHome') {
                                  _ctr.playMusic('PodcastHome', audioList.data, index);
                                }
                                Get.to(() => MusicPlayer());
                              } else {
  ...
  Future<AudioPlayListModel> getAudioFiles() async {
    ...
    return apiInterface.getAllPlaylist(token!, "Playlists", "");
  }
}
